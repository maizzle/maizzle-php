...
module.exports.processEmails = (config, build_path, env) => {
  ...
  files.map((file) => {
    ...
    if (config.transforms.prettify) {
      html = pretty(html, {ocd: true, indent_inner_html: false});
    }

    if (config.transforms.minify.enabled) {
      html = minify(html, {
        html5: false,
        maxLineLength: config.transforms.minify.maxLineLength || 500,
        keepClosingSlash: true,
        collapseWhitespace: config.transforms.minify.collapseWhitespace ? true : false,
        removeEmptyAttributes: true,
        preserveLineBreaks: config.transforms.minify.preserveLineBreaks ? true : false,
